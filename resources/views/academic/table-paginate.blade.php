    ...
    <tbody>
        @forelse ($academics as $key => $item)
            <tr>
                ...
                <td>
                    <form action="{{ route('academics.destroy', $item->id) }}"
                        method="POST">
                        <a href="{{ route('academics.show', $item->id) }}"
                            class=" btn btn-sm btn-success"><i class="fa fa-eye text-light"
                                aria-hidden="true"></i></a>
                        <a href="{{ route('academics.edit', $item->id) }}"
...
<div class="academics_paginate float-right mt-4 ">
    {!! $academics->appends($_GET)->links() !!}
</div>

    ...
    <tbody>
        @forelse ($degrees as $key => $item)
            <tr>
                ...
                <td>
                    <form action="{{ route('degrees.destroy', $item->id) }}"
                        method="POST">
                        <a href="{{ route('degrees.show', $item->id) }}"
                            class=" btn btn-sm btn-success"><i class="fa fa-eye text-light"
                                aria-hidden="true"></i></a>
                        <a href="{{ route('degrees.edit', $item->id) }}"
...
<div class="degrees_paginate float-right mt-4 ">
    {!! $degrees->appends($_GET)->links() !!}
</div>
